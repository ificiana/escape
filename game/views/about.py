...
def get_about_view_ui(window: arcade.Window) -> arcade.gui.UIWidget:
    ...
    names=[
        "Vagish (Vagish Vela)","Tacchan (Arkaprabha Sinha Roy)",
        "Shahnawaz28 (Shahnawaz Hussain)","Arpi369 (Arpita Shaw)",
        "Gods_gift (Veenu Chhabra)","Deb01 (Mainak Deb)",
    ...
    for i in range(3):
        ...
        v_box.add(h_box)
    name = arcade.gui.UIFlatButton(
                text=names[-1], width=350, style=name_style
            )
    v_box.add(name)
